...
from medsafe.api.routes import suggest as suggest_routes
...
from medsafe.resolution.combinations import load_combinations
...
def build_engine_state(
    ...
    combinations = load_combinations(settings.combinations_file)
    matcher = Matcher(
        repository,
        blocklist,
    ...
    if combinations.missing:
        logger.warning("Combination index missing at %s", settings.combinations_file)
    return EngineState(
        settings=settings,
