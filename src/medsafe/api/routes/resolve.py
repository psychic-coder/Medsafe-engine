...
def _resolve_one(
    ...
    # A combination is identified but not substitutable. It gets an explicit out_of_scope_fdc
    # result rather than a null substitution block, because null reads as "we did not look" and
    # this is a decision: swapping one ingredient of a combination is refused on purpose.
    if include_substitutes and result.combination is not None:
        substitution = SubstitutionResult(
            status=SubstitutionStatus.OUT_OF_SCOPE_FDC,
            molecule_id=result.combination.components[0].molecule_id,
            notes=(
                f"{result.combination.brand_raw!r} contains "
                f"{len(result.combination.components)} active ingredients; substituting on one "
                "component of a combination is unsafe, so v1 refuses rather than partially "
                "substituting",
            ),
        )
    elif include_substitutes and result.match is not None:
        ...
        substitution = find_substitutes_for_molecule(
            ...
            strength_hint=result.normalized.strength_hint,
        )
