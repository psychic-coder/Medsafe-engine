...
from medsafe.resolution.matcher import ResolutionResult
...
def check(
    ...
    inputs = [entry for result in results for entry in _expand(result)]
    ...
    return CheckResponse.from_domain(report, resolutions, coverage_missing=coverage.missing)


def _expand(result: ResolutionResult) -> list[MoleculeInput]:
    """One resolution to the molecules it contributes to the pairwise set.

    A combination pack expands to one entry per ingredient, so every ingredient is paired against
    every other drug. Collapsing it to a single entry would drop the other ingredients out of the
    check while the report still said "checked" — the coverage lie this endpoint exists to prevent,
    reached through resolution rather than through coverage.
    """
    if result.combination is not None:
        return [
            MoleculeInput(
                ...
                inn_name=component.inn_name,
                resolved=True,
                from_combination=result.combination.brand_raw,
            )
            for component in result.combination.components
        ]
