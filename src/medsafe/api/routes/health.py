...
def readiness(request: Request, response: Response) -> ReadinessResponse:
    ...
    combinations_loaded = not state.combinations.missing
    ...
    if not combinations_loaded:
        notes.append(
            "combination index is missing — multi-ingredient pack names will not resolve; "
            "run scripts/build_brand_aliases.py"
        )
    ...
    checks = {
        ...
        "combinations_loaded": combinations_loaded,
    }
    ...
    return ReadinessResponse(
        ...
        coverage_manifest_loaded=coverage_loaded,
        combinations_loaded=combinations_loaded,
        combination_brands=len(state.combinations),
        checks=checks,
        notes=tuple(notes),
    )
