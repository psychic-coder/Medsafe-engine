...
from medsafe.resolution.combinations import Combination, CombinationIndex, load_combinations
...
class ResolvedCombination:
    """A pack name identified as containing two or more molecules.

    Deliberately not a :class:`ResolvedMatch`. A match carries exactly one molecule and licenses
    substitution; a combination carries several and forbids it, so giving them the same type would
    let a caller price a two-drug product against one-drug products by accident.
    """

    brand_raw: str
    brand_key: str
    components: tuple[MoleculeRef, ...]

    def __post_init__(self) -> None:
        if len(self.components) < 2:
            raise ValueError("A combination must carry at least two components")
...
class ResolutionResult:
    ...
    combination: ResolvedCombination | None = None
    ...
    def __post_init__(self) -> None:
        ...
        # Same shape of invariant as `match`, for the same reason: a combination is an
        # identification, so it must be impossible to carry one without saying so in `status`, or
        # to claim the status without the components that justify it.
        if self.status is ResolutionStatus.COMBINATION:
            if self.combination is None:
                raise ValueError("A combination result must carry its components")
        elif self.combination is not None:
            raise ValueError("Only a combination result may carry components")

    @property
    def is_resolved(self) -> bool:
        """True only for a single-molecule identification. A combination is not `resolved`."""
        return self.status is ResolutionStatus.RESOLVED

    @property
    def is_identified(self) -> bool:
        """True when the engine knows what the drug is, as one molecule or as several.

        The distinction from :attr:`is_resolved` is load-bearing: interaction checking wants every
        molecule it can name, so it asks this; substitution wants exactly one, so it asks the other.
        """
        return self.status in (ResolutionStatus.RESOLVED, ResolutionStatus.COMBINATION)

    @property
    def molecule_refs(self) -> tuple[MoleculeRef, ...]:
        """Every molecule this input names — one for a match, several for a combination."""
        if self.match is not None:
            return (self.match.molecule,)
        if self.combination is not None:
            return self.combination.components
        return ()
...
class Matcher:
    ...
    def __init__(
        ...
        *,
        ...
        combinations: CombinationIndex | None = None,
    ) -> None:
        ...
        self.combinations = combinations if combinations is not None else load_combinations()
    ...
    def resolve(self, raw: str) -> ResolutionResult:
        ...
        # (3) Combination pack name — auto-accept, but as several molecules rather than one.
        #
        # It sits after the single-molecule paths on purpose. If a name is both a real INN and a
        # combination brand, the INN reading is the safer one: it identifies exactly what the name
        # literally says, whereas the combination reading would silently add a molecule the user
        # never typed. It sits before fuzzy for the ordinary reason — a known name must never fall
        # through to guessing.
        combination = self.combinations.get(key)
        if combination is not None:
            resolved_components = self._combination_components(combination)
            if len(resolved_components) == len(combination.components):
                return ResolutionResult(
                    query=raw,
                    normalized=normalized,
                    status=ResolutionStatus.COMBINATION,
                    combination=ResolvedCombination(
                        brand_raw=combination.brand_raw,
                        brand_key=combination.brand_key,
                        components=resolved_components,
                    ),
                    notes=(
                        "this pack contains more than one active ingredient; every ingredient is "
                        "checked, and it cannot be swapped for a single-ingredient generic",
                    ),
                )
            # A component that is not in the graph would make the expansion silently incomplete,
            # and an incomplete expansion reports as "checked". Refuse the whole combination.
            missing = len(combination.components) - len(resolved_components)
            return ResolutionResult(
                query=raw,
                normalized=normalized,
                status=ResolutionStatus.UNRESOLVED,
                notes=(
                    f"{combination.brand_raw!r} is a combination pack, but {missing} of its "
                    f"{len(combination.components)} ingredients are not in the catalogue, so it "
                    "cannot be checked completely and has not been accepted",
                ),
            )

        # (4) Fuzzy — CANDIDATES ONLY. This branch cannot produce a match.
    ...
    def _combination_components(self, combination: Combination) -> tuple[MoleculeRef, ...]:
        """Look each component up in the graph, dropping any the catalogue does not have."""
        wanted = [component.molecule_id for component in combination.components]
        found = {
            str(record["molecule_id"]): MoleculeRef.from_record(record)
            for record in self.repository.get_molecules(wanted)
        }
        # Order follows the pack, not the graph, so the components read as they are printed.
        return tuple(found[mid] for mid in wanted if mid in found)
...
def resolved_molecule_ids(results: Sequence[ResolutionResult]) -> list[str]:
    """Every identified molecule id, including each component of a combination.

    Candidates never contribute: a fuzzy suggestion is not an identification, so it must not reach
    a caller that treats this list as "the drugs we know the patient is taking".
    """
    return [
        ref.molecule_id
        for result in results
        for ref in result.molecule_refs
    ]
