...
class NormalizedName:
    ...
    """A bare number in the name with no unit — the ``500`` in "Glycomet 500".

    Indian pack names put the strength here almost without exception, and it is the single most
    common thing a patient types after the brand. It is kept separate from ``strength_value``
    because a number with no unit is a hint, not a measurement: it could be milligrams, a pack
    count, or part of the brand ("Ecosprin 75", "B 12"). Callers may use it to prefer one strength
    over another, never to assert one.

    It does not enter ``normalized``. The bare number was already discarded from the comparison key
    before this field existed, and it still is, so nothing about matching or auto-accept changes.
    """
...
def normalize(raw: str) -> NormalizedName:
    ...
    for token in raw_tokens:
        ...
        if re.fullmatch(r"\d+(?:\.\d+)?", token):
            # Recorded, not kept: the token still leaves the comparison key, exactly as before.
            # Only the first is taken, so "Ecosprin 75 10" keeps the strength and not the pack size.
            if strength_hint is None and strength_value is None:
                strength_hint = float(token)
            continue
