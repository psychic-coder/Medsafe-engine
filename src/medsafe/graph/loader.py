...
brand_aliases.csv    raw_string, normalized_string, source, molecule_id  (same shape; brand pack)
...
BRAND_ALIASES_FILE = "brand_aliases"
...
STAGE_FILES: dict[str, tuple[str, ...]] = {
    ...
    # A second alias stage rather than another stem on the first. Stage resolution takes the first
    # filename that exists, so listing brand_aliases alongside the bridge table would load one or
    # the other; they are both wanted, and they come from different pipelines with different review
    # histories, so a load report that distinguishes them is worth a stage.
    BRAND_ALIASES_FILE: ("brand_aliases",),
...
class ArtifactSet:
    ...
    brand_aliases: list[Record] = field(default_factory=list)
...
def _normalize_keys(stage: str, rows: list[Record]) -> list[Record]:
    ...
    if stage in (ALIASES_FILE, BRAND_ALIASES_FILE):
        prepared = []
        for row in rows:
...
def _merge_stage(
    ...
    merge = {
        ...
        BRAND_ALIASES_FILE: repo.merge_aliases,
...
def load_records(
    ...
    *,
    ...
    _merge_stage(repo, report, BRAND_ALIASES_FILE, artifacts.brand_aliases, strict)
...
def load_artifacts(
    ...
    *,
    ...
    _merge_stage(
        repo, report, BRAND_ALIASES_FILE, read_stage(directory, BRAND_ALIASES_FILE), strict
    )
