...
class MoleculeInput:
    ...
    def display(self) -> str:
        """Label plus the pack it came out of, for components expanded from a combination.

        A report listing "amoxicillin" and "clavulanic acid" separately is correct but puzzling to
        someone who typed "Augmentin" once; naming the pack restores the connection.
        """
        if self.from_combination and self.inn_name:
            return f"{self.inn_name} (in {self.from_combination})"
        return self.label
