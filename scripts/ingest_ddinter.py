...
becomes invisible downstream.
    ...
    python scripts/ingest_ddinter.py --input data/raw/ddinter_interactions.csv \\
        --atc data/raw/ddinter_atc.csv

The manifest is written **every run, unconditionally**, even when no ATC file is supplied — in that
case it records an empty molecule mapping, which makes every downstream pair report "not checked".
That is the intended behaviour: an unknown coverage picture must degrade to "we did not check",
never to silence.
"""
